...
public class DataIntegrityTest {
    ...
    for (String str : assets.poses.keySet()) {
      ...
      for (Pose p : pl) {
        for (String slot : p.getListOfSlots()) {
          if (p.getItems(slot) == null) continue;

          for (Item i : p.getItems(slot)) {
            if (i.sprite.equals("")) continue;
            ...
            if (i.mask.equals("") || i.mask.equals("self")) continue;
  ...
  public void checkItemsCanRenderInPose() {
    ...
    for (String str : assets.poses.keySet()) {
      ...
      for (Pose pose : poseList) {
        for (String slot : pose.getListOfSlots()) {
          if (pose.getItems(slot) == null) continue;
          ...
          for (String renderslot : spriteRenderSlots) {
            if (
              "offhanda".equals(renderslot) || "offhandw".equals(renderslot)
            ) {
              if (
                !pose.renderorder.contains("offhand") &&
                !pose.renderorder.contains(renderslot)
              ) {
                errors.add(
                  "Pose list " +
                  str +
                  ...
                  ": slot " +
                  slot +
                  ": renderslot offhand - slot offhand or " +
                  renderslot +
                  " not found in #renderorder"
                );
              }
    ...
    Assert.assertTrue(
      "Some items with sprites don't have their slot defined in the #renderorder",
      errors.isEmpty()
    );
  }
  ...
  public void checkNeeds() {
    ...
    for (String str : assets.poses.keySet()) {
      ...
      for (Pose pose : poseList) {
        ...
        if (race.isEmpty()) {
          System.out.println(
            "no race found for pose " + pose.name + " " + pose.roles
          );
        }
