...
public class Summary {
  ...
  private static String summarizeMilitary(Nation n, NationFeatures features) {
    ...
    for (List<Unit> list : n.getListsOfType("infantry", false, true)) {
      ...
      if (list.get(0).getTotalProt() < 12) lightinf += list.size();
      else heavyinf += list.size();
    }

    for (List<Unit> list : n.getListsOfType("ranged", false, true)) {
      for (Unit u : list) {
        if (
          u.tags.containsName("elite") &&
          !elites.contains(u.name.type.toString())
        ) {
          elites.add(u.name.type.toString());
        } else if (!rangeds.contains(u.name.type.toString())) rangeds.add(
          u.name.type.toString()
        );
      }
    }
      ...
      if (list.get(0).getTotalProt() < 12) {
        ...
        for (Unit u : list) {
          ...
          if (u.getSlot("mount") != null) {
            for (Arg value : u.getSlot("mount").tags.getAllValues("animal")) {
              ...
              if (!lightmounts.contains(mounttype)) lightmounts.add(mounttype);
            }
          }
        }
      } else {
        ...
        for (Unit u : list) {
          if (u.getSlot("mount") != null) {
            for (Arg value : u.getSlot("mount").tags.getAllValues("animal")) {
              String mounttype = value.get();
              if (!heavymounts.contains(mounttype)) heavymounts.add(mounttype);
            }
          }
        }
      }
    ...
    n
      ...
      .forEach(sacredunit -> {
        ...
        if (sacredunit.race != n.races.get(0)) sacred
          .append(sacredunit.race.visiblename)
          .append(" ");

        if (sacredunit.getTotalProt() >= 12) sacred.append("heavy ");
        else sacred.append("light ");

        if (
          (sacredunit.pose.roles.contains("mounted") ||
            sacredunit.pose.roles.contains("sacred mounted")) &&
          sacredunit.getSlot("mount") != null
        ) {
          String mount = sacredunit
            .getSlot("mount")
            .tags.getString("animal")
            .orElseThrow();
          ...
          sacredunit.pose.roles.contains("infantry") ||
          sacredunit.pose.roles.contains("sacred infantry")
        ) sacred.append("infantry");
        else if (
          sacredunit.pose.roles.contains("chariot") ||
          sacredunit.pose.roles.contains("sacred chariot")
    ...
    if (lightinf > 0) infantryList.add("light infantry");
    if (heavyinf > 0) infantryList.add("heavy infantry");

    if (!infantryList.isEmpty()) military
      .append(Generic.capitalizeFirst(String.join(" and ", infantryList)))
      .append(". ");

    // ranged
    if (!rangeds.isEmpty()) military
      .append(
        Generic.capitalizeFirst(
          NameGenerator.writeAsList(rangeds, " and ", NameGenerator::plural)
        )
      )
      .append(". ");
    ...
    if (lightcav > 0) {
      String lightmount = NameGenerator.writeAsList(
        lightmounts
          .stream()
      ...
      cavDescriptions.add(
        "light" +
        (lightmount.equals("horse") ? "" : (" " + lightmount)) +
        " cavalry"
      );
    }
    if (heavycav > 0) {
      String heavyMount = NameGenerator.writeAsList(
        heavymounts
          .stream()
          .map(m -> String.join(" ", Generic.parseArgs(m, "'")))
          .collect(Collectors.toList()),
      ...
      cavDescriptions.add(
        "heavy" +
        (heavyMount.equals("horse") ? "" : (" " + heavyMount)) +
        " cavalry"
      );
    }

    if (!cavDescriptions.isEmpty()) military
      .append(
        Generic.capitalizeFirst(
          NameGenerator.writeAsList(cavDescriptions, " and ")
        )
      )
      .append(". ");

    if (chariots > 0) military.append("Chariots. ");

    // elites
    if (elites.size() > 0) military
      .append(
        Generic.capitalizeFirst(
    ...
    military
      .append("Sacred ")
      .append(NameGenerator.writeAsList(sacreds, false))
      .append(".");

    if (features.fortcost < 0) military.append(" Cheaper forts.");
    if (features.fortcost > 0) military.append(" Expensive forts.");
  ...
  private static String summarizeRace(Nation n) {
    ...
    output.append(
      Generic.capitalize(NameGenerator.plural(n.races.get(0).visiblename))
    );
    ...
    n
      ...
      .forEach(r -> {
        ...
        if (
          amount > 1 && !r.visiblename.equals(n.races.get(0).visiblename)
        ) output
          .append(", some ")
          .append(Generic.capitalize(NameGenerator.plural(r.visiblename)));
      });
    ...
    n
      ...
      .forEach(r ->
        output
          .append(", ")
          .append(Generic.capitalize(r.name))
          .append(" auxillaries")
      );
    ...
    for (Command c : n.getCommands()) {
      if (c.command.equals("#idealcold")) {
        ...
        if (cold < 0) output
          .append(", prefers Heat scale +")
          .append(Math.abs(cold));
        else if (cold > 0) output
          .append(", prefers Cold scale +")
          .append(Math.abs(cold));
      }
    }
  ...
  private static MagicPathInts findMaxPaths(Nation n) {
    ...
    n
      ...
      .forEach(u -> {
        ...
        for (MagicPath path : MagicPath.values()) if ( // Get rid of single randoms that don't reinforce non-random paths
          paths.get(path) != 0 || rpaths.get(path) > 1
        ) paths.set(path, rpaths.get(path)); // If a non-random path is not zero or randoms aren't linking // use randoms and non-randoms on that path
  ...
  private static String summarizeMagic(
    Nation n,
    MagicPathInts maxPaths,
    NationFeatures features
  ) {
    ...
    for (MagicPath path : MagicPath.NON_HOLY) {
      if (
        maxPaths.get(path) > 2
      ) strong.add(path.name); // Strong if 3+ available
      else if (
        maxPaths.get(path) > 0
      ) weak.add(path.name); // Weak if 1-2 is available
    }

    StringBuilder magic = new StringBuilder("Magic: ");

    if (strong.size() > 0) magic
      .append(NameGenerator.writeAsList(strong, true))
      .append(".");

    if (weak.size() > 0) magic
      .append(" Weak ")
      .append(NameGenerator.writeAsList(weak, true))
      .append(".");
    ...
    if (primarydrainimmune) {
      ...
      n
        .selectCommanders("mages-2")
        .findFirst()
        .ifPresent(u ->
          magic
            .append(n.comlists.get("mages-2").get(0).name)
            .append(" is not affected by Drain scale.")
        );
    }
    ...
    // Lab cost
    if (features.labcost != 500) magic
      .append(" Laboratories cost ")
      .append(features.labcost)
      .append(" gold.");

    return magic.toString();
  }

  private static String summarizePriests(
    MagicPathInts maxPaths,
    NationFeatures features
  ) {
    // Priests;
    ...
    if (maxPaths.get(MagicPath.HOLY) == 1) priest.append("Weak");
    else if (maxPaths.get(MagicPath.HOLY) == 2) priest.append("Moderate");
    else if (maxPaths.get(MagicPath.HOLY) >= 3) priest.append("Strong");

    if (features.nopreach) priest.append(
      ", Dominion does not spread unless blood is sacrificed"
    );
    if (features.templecost != 400) priest
      .append(", temples cost ")
      .append(features.templecost)
      .append(" gold");
    if (features.reanim) priest.append(", can reanimate the dead");
    if (features.bloodsac) priest.append(", can perform blood sacrifices");
    if (features.manikin) priest.append(", can reanimate manikin");
    if (features.golemcult > 0) priest.append(
      ", their constructs are stronger within their Dominion"
    );
  ...
  private static class NationFeatures {
    ...
    private NationFeatures(Nation n) {
      for (Command c : n.commands) {
        ...
        if (c.command.equals("#manikinreanim")) manikin = true;
        if (c.command.equals("#golemhp")) golemcult = c.args.get(0).getInt();
        if (c.command.equals("#templecost")) templecost = c.args
          .get(0)
          .getInt();
        if (c.command.equals("#labcost")) labcost = c.args.get(0).getInt();
        if (c.command.equals("#fortcost")) fortcost = c.args.get(0).getInt();
      }
    }
  }
}
