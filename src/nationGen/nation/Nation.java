...
public class Nation {
  ...
  private void getColors() {
    ...
    int direction = -1;
    if (random.nextDouble() < 0.5) direction = 1;

    int amount = random.nextInt(40) + 30;

    colors[1] = new Color(
      Math.min(255, Math.max(0, colors[0].getRed() + direction * amount)),
      Math.min(255, Math.max(0, colors[0].getGreen() + direction * amount)),
      Math.min(255, Math.max(0, colors[0].getBlue() + direction * amount))
    );
  ...
  private void getRaces() {
    ...
    for (Command command : this.races.get(0).tags.getAllCommands(
        "secondaryracecommand"
      )) {
  ...
  private void generateSacreds() {
    ...
    if (random.nextDouble() > 0.8) {
      int power = 0;
      if (random.nextDouble() > 0.25) power++;
      if (random.nextDouble() > 0.25) power++;
      if (random.nextDouble() > 0.5) power++;
      if (random.nextDouble() > 0.75) power++;
      if (random.nextDouble() > 0.9) power++;
      ...
      String role = "";
      if (
        sacred.pose.roles.contains("infantry") ||
        sacred.pose.roles.contains("elite infantry") ||
        sacred.pose.roles.contains("sacred infantry")
      ) role = "infantry";
      else if (
        sacred.pose.roles.contains("mounted") ||
        sacred.pose.roles.contains("elite mounted") ||
        sacred.pose.roles.contains("sacred mounted")
      ) role = "mounted";
      else if (
        sacred.pose.roles.contains("ranged") ||
        sacred.pose.roles.contains("elite ranged") ||
        sacred.pose.roles.contains("sacred ranged")
      ) role = "ranged";
      else if (
        sacred.pose.roles.contains("chariot") ||
        sacred.pose.roles.contains("elite chariot") ||
        sacred.pose.roles.contains("sacred chariot")
      ) role = "chariot";

      if (role.equals("")) System.out.println(
        sacred.race +
        " " +
        sacred.pose.roles +
        " AS ELITE UNIT HAS RECEIVED NO ROLES!"
      );

      if (sacred != null) {
        if (unitlists.get(role) != null) System.out.print(
          role + " - " + unitlists.get(role).size() + " -> "
        );
      ...
      if (random.nextDouble() < 0.9) {
        ...
        if (random.nextDouble() < 0.8) {
          ...
          if (
            ...
            (sacredcount < 3 || random.nextDouble() < 0.5)
          ) {
            power++;
            if (
              random.nextDouble() < 0.5 &&
              (sacredcount < 2 || random.nextDouble() < 0.5)
            ) {
              power++;
              if (
                random.nextDouble() < 0.4 &&
                (sacredcount < 1 || random.nextDouble() < 0.5)
              ) {
                power++;
  ...
  private void generateScouts() {
    ...
    if (
      races.get(0).hasRole("scout") &&
      !races.get(0).tags.containsName("#no_scouts")
    ) comlists.get("scouts").add(scoutgen.generateScout(races.get(0)));
    else if (
      races.get(1).hasRole("scout") &&
      !races.get(1).tags.containsName("#no_scouts")
  ...
  private void generateSpecialComs() {
    ...
    double specialcomchance = races
      .get(0)
      .tags.getDouble("specialcommanderchance")
      .orElse(0.05);

    if (random.nextDouble() < specialcomchance) {
      SpecialCommanderGenerator scg = new SpecialCommanderGenerator(
        this,
        nationGen,
        assets
      );
  ...
  private void getForts() {
    ...
    // Forts
    List<Filter> possibleForts = ChanceIncHandler.retrieveFilters(
      "forts",
      "default_forts",
      assets.miscdef,
      null,
    ...
    possibleForts = ChanceIncHandler.getFiltersWithType(
      "era " + era,
      possibleForts
    );
  ...
  private void generateMonsters() {
    // Monsters
    double monsterchance = races
      .get(0)
      .tags.getDouble("monsterchance")
      .orElse(0.05);
  ...
  private void getStartAffinity() {
    ...
    List<Filter> posaff = ChanceIncHandler.retrieveFilters(
      "startaffinities",
      "startaffinities",
      assets.miscdef,
      null,
  ...
  public static void handleCommand(
    ...
    if (
      ...
      String value = nationGen.units.GetValue(
        copystats + "",
  ...
  public void generate(List<NationRestriction> restrictions) {
    ...
    if (
      !checkRestrictions(
        ...
        RestrictionType.MagicDiversity,
        RestrictionType.PrimaryRace
      )
    ) {
      return;
    ...
    if (
      !checkRestrictions(
        ...
        RestrictionType.UnitRace,
        RestrictionType.NoUnitOfRace
      )
    ) {
      return;
    }
  ...
  private void addNationThemes() {
    ...
    for (String str : race.tags.getAllStrings("freenationtheme")) {
      List<Theme> frees = ChanceIncHandler.getFiltersWithType(
        str,
        possibleThemes
      );
    ...
    for (int i = 0; i < guaranteedthemes; i++) {
      possibleThemes = ChanceIncHandler.getValidFilters(
        possibleThemes,
        this.nationthemes
      );
  ...
  private void addRaceThemes(Race race) {
    ...
    for (String str : race.tags.getAllStrings("freetheme")) {
      ...
      if (frees.isEmpty()) {
        throw new IllegalStateException(
          race + " has #freetheme " + str + " but no filters of #type " + str
        );
      }
    ...
    // TODO: This is unused
    boolean getsNonFreeThemes =
      race == races.get(0) ||
      race.tags.containsName("normal_themes_as_secondary");
  ...
  public void finalizeUnits() {
    List<Command> conditional =
      this.races.get(0)
        .tags.getAllValues("secondaryracecommand_conditional")
        .stream()
        .map(Arg::getCommand)
        .collect(Collectors.toList());
    ...
    for (List<Unit> l : this.comlists.values()) for (Unit u : l) {
      ...
      if (priest > 0) {
        Generic.getAllNationTags(this)
          .getAllValues("priestextracost")
          .stream()
          .map(Arg::getInt)
    ...
    for (List<Unit> l : this.unitlists.values()) for (Unit u : l) {
  ...
  private void applyNationWideFilter() {
    ...
    if (random.nextDouble() < 0.33) {
      count++;
      if (random.nextDouble() < 0.33) count++;
    }

    List<Filter> possibles = ChanceIncHandler.retrieveFilters(
      "nationwidefilters",
      "default_nationwidefilters",
      assets.filters,
      null,
      races.get(0)
    );
    ...
    if (random.nextDouble() < this.percentageOfRace(races.get(1))) {
      ...
      if (possibles.size() > 0 && random.nextDouble() < 0.1) {
        ...
        primary = false;
        possibles = ChanceIncHandler.retrieveFilters(
          "nationwidefilters",
          "default_nationwidefilters",
          assets.filters,
          null,
    ...
    for (int i = 0; i < count; i++) {
      ...
      for (List<Unit> l : this.comlists.values()) for (Unit u : l) {
        if (
          ((u.race == races.get(0)) == primary || both) &&
          ChanceIncHandler.canAdd(u, f) &&
          !f.tags.containsName("trooponly")
        ) {
          added++;
      ...
      for (List<Unit> l : this.unitlists.values()) for (Unit u : l) {
        if (
          ((u.race == races.get(0)) == primary || both) &&
          ChanceIncHandler.canAdd(u, f) &&
          !f.tags.containsName("commanderonly")
        ) {
          added++;
          u.appliedFilters.add(f);
        }
  ...
  private void giveSecondaryRaceSpecialCommands(
    Unit u,
    List<Command> conditional
  ) {
    if (u.race == races.get(0)) return;
  ...
  public List<List<Unit>> getMagesInSeparateLists() {
    List<Unit> all =
      this.selectCommanders("mages").collect(Collectors.toList());
  ...
  public List<String> writeLines(String spritedir) {
    ...
    lines.add(
      ...
      races.get(0) +
      " race themes: " +
      races.get(0).themefilters
    );
    lines.add(
      "-- Generated with " +
      races.get(1) +
      " race themes: " +
      races.get(1).themefilters
    );
    lines.add(
      "---------------------------------------------------------------"
    );
    ...
    lines.add(
      "#descr \"" +
      "A glorious NationGen nation! Generated from seed " +
      seed +
      " with settings integer " +
    ...
    // Gods
    for (Command cmd : this.gods) lines.add(cmd.toModLine());
  ...
  public List<ShapeChangeUnit> getShapeChangeUnits() {
    ...
    for (ShapeChangeUnit su : nationGen.forms) {
      if (units.contains(su.otherForm)) sus.add(su);
    }
  ...
  private List<String> writeRecLines(
    boolean coms,
    Map<String, List<Unit>> unitlists
  ) {
    List<String> order = Generic.parseArgs(
      "ranged infantry mounted chariot special sacred monsters"
    );
    if (coms) order = Generic.parseArgs(
      "scouts commanders specialcoms priests mages"
    );

    String line = "#addreccom";
    if (!coms) line = "#addrecunit";
  ...
  private static <T> Stream<T> flattenListsWithPrefix(
    Map<String, ? extends Collection<T>> map,
    String prefix
  ) {
    return map
      .entrySet()
  ...
  public Stream<Unit> selectTroopsWithMontagGuesses(String type) {
    return Stream.concat(
      selectTroops(type),
      selectTroops("montagtroops").filter(u -> u.guessRole().equals(type))
    );
  }
  ...
  public List<List<Unit>> getListsOfType(
    String type,
    boolean coms,
    boolean troops
  ) {
  ...
  public List<String> writeSitesLines() {
    ...
    for (Site site : sites) lines.addAll(site.writeLines());
    return lines;
  }
