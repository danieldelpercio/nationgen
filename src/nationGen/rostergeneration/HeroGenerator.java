...
public class HeroGenerator {
  ...
  public List<Unit> generateHeroes(int count) {
    ...
    while (units.size() < count) {
      Unit u;
      if (n.random.nextDouble() < 0.5) u = (generateMageHero(false));
      else u = (generateHero(false));
  ...
  private Unit generateHero(boolean multihero) {
    ...
    int power = 5;
    if (multihero) power += n.random.nextInt(6); // +0-5;
    else power += 3 + n.random.nextInt(8); // +3-10;
    ...
    if (n.races.contains(hero.race)) hero.color = n.colors[1];
    else hero.color = n.colors[2];

    if (n.random.nextBoolean()) hero.color = Drawing.getColor(n.random);

    if (n.random.nextBoolean()) {
      int tier = 1;
      if (n.random.nextBoolean()) tier = 2;

      MageGenerator mg = new MageGenerator(ng, n, assets);
      mg.setIdentifier("herogen");

      MagicPattern pat = chandler
        .handleChanceIncs(
          hero,
          MageGenerator.getPatternsOfLevel(mg.possiblePatterns, tier)
        )
        .getRandom(n.random);
  ...
  private Unit generateMageHero(boolean multihero) {
    ...
    int tier = 3;
    if (multihero && n.random.nextDouble() < 0.5) tier = 2;
    ...
    if (n.races.contains(r)) hero.color = n.colors[1];
    else hero.color = n.colors[2];
    ...
    int power = 2;
    if (!multihero) power += 2 + n.random.nextInt(7); // 2-8;
    else power += n.random.nextInt(5); // 0-4;
  ...
  private MagicPattern generatePattern(boolean multihero) {
    ...
    int picks = 4;
    if (multihero) picks += n.random.nextInt(5); // +0-4
    else picks += 2 + n.random.nextInt(5); // +2-6
    int maxpicks = picks;
    ...
    while (picks > 0) {
      ...
      if (picks - level < 0) level = picks;

      if (
        n.random.nextDouble() > picks / maxpicks &&
        level > 1 &&
        ((level - 1) / picks > 0.25 || picks < 4)
      ) level--;
      else if (level == 4 && n.random.nextDouble() < 0.8) level--;

      if (n.random.nextDouble() > picks / maxpicks && multihero) break;
    }

    while (picks > 0) {
      int randoms = 15;
      if (n.random.nextDouble() < 0.75) randoms -= 8;
      if (n.random.nextDouble() < 0.75) randoms -= 4;
      ...
      if (amount == 1 && n.random.nextDouble() < 0.1) amount = 0.5;
  ...
  private boolean hasRole(Race r, String role) {
    for (Pose p : r.poses) {
      if (p.roles.contains(role)) return true;
    }
