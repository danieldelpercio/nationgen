...
public class IdHandler {
  ...
  private String parseOne(List<String> args, String line, String c) {
    ...
    if (end < 0 || end > line.length() + 1) end = line.length();
    ...
    for (String line : FileUtil.readLines(filename)) {
      ...
      if (command.equals("nation")) {
        if (args.get(0).contains("-")) {
          for (
            int i = Integer.parseInt(args.get(0).split("-")[0]);
            i <= Integer.parseInt(args.get(0).split("-")[1]);
            i++
          ) {
  ...
  public int nextNationId() {
    nation++;
    while (forbiddenNations.contains(nation)) nation++;
    return nation;
  }

  public int nextUnitId() {
    unit++;
    while (forbiddenUnits.contains(unit)) unit++;
  ...
  public int nextWeaponId() {
    weapon++;
    while (forbiddenWeapons.contains(weapon)) weapon++;
    return weapon;
  }

  public int nextSiteId() {
    site++;
    while (forbiddenSites.contains(site)) site++;
    return site;
  }

  public int nextArmorId() {
    armor++;
    while (forbiddenArmors.contains(armor)) armor++;
    return armor;
  }

  public int nextNameId() {
    name++;
    while (forbiddenNames.contains(name)) name++;
    return name;
  }

  public int nextMontagId() {
    montag++;
    while (forbiddenMontags.contains(montag)) montag++;
    return montag;
  }
}
