...
public class CoriderUnit extends Unit {

  public Unit otherForm;
  public Corider coriderForm;
  boolean sacred = false;
  private int gcost = 0;
  private NationGenAssets assets;

  // otherForm is the complete main form that is recruited (rider), whereas coriderForm is the actual, corider on its own
  public CoriderUnit(
    NationGen nationGen,
    ...
    this.coriderForm = coriderForm.getCopy();
  }
  ...
  public void polish(NationGen n, Nation nation) {
    ...
    if (otherForm != null) {
      for (Command c : otherForm.getCommands()) {
        if (
          c.command.equals("#holy") && !coriderForm.tags.containsName("corider")
        ) {
          sacred = true;
        } else if (
          c.command.equals("#holy") && coriderForm.tags.containsName("corider")
    ...
    for (Command c : coriderForm.commands) {
      ...
      for (Filter f : otherForm.appliedFilters) {
        ...
        for (Command c : f.getCommands()) {
          if (
            ...
            !coriderForm.tags.containsName("corider")
          ) {
            sf.commands.add(c);
  ...
  public void writeSprites(String spritedir) {
    ...
    for (Command c : coriderForm.commands) {
      ...
      if (c.command.equals("#spr1")) {
        ...
        FileUtil.writeTGA(
          spr1,
          "/mods/" + spritedir + "/corider_" + id + "_a.tga"
        );
      }
    }
    ...
    for (Command c : coriderForm.commands) {
      if (c.command.equals("#spr2")) {
        ...
        if (c.args.get(0).get().equals("shift")) {
          if (spr1 == null) {
            throw new IllegalStateException(
              "Can't shift attack sprite because no #spr1 command found for corider unit!"
            );
          }
        ...
        FileUtil.writeTGA(
          spr2,
          "/mods/" + spritedir + "/corider_" + id + "_b.tga"
        );
      }
    }
  }
  ...
  public List<String> writeLines(String spritedir) {
    ...
    if (otherForm != null) lines.add(
      "--- Corider form for " + otherForm.getName()
    );
    ...
    if (coriderForm.commands.stream().anyMatch(c -> c.command.equals("#spr1"))) {
      lines.add("#spr1 \"" + spritedir + "/corider_" + id + "_a.tga" + "\"");
    }
  
    if (coriderForm.commands.stream().anyMatch(c -> c.command.equals("#spr2"))) {
      lines.add("#spr2 \"" + spritedir + "/corider_" + id + "_b.tga" + "\"");
    }

    if (coriderForm.keepname && otherForm != null) lines.add(
      "#name \"" + otherForm.name + "\""
    );
