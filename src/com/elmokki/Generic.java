...
public class Generic {
  ...
  public static <E extends Entity> E getRandom(List<E> list, Random r) {
    double max = 0;
    for (E e : list) max = max + e.basechance;
  ...
  public static Tags getAllUnitTags(Unit u) {
    ...
    for (Theme t : u.race.themefilters) tags.addAll(t.tags);
  ...
  public static Tags getAllNationTags(Nation n) {
    ...
    if (n.races.get(0) != null) {
      ...
      for (Theme t : n.races.get(0).themefilters) tags.addAll(t.tags);
    }
    for (Filter f : n.nationthemes) tags.addAll(f.tags);
  ...
  public static String plural(String line) {
    ...
    "ies";
      ...
      line = line + "ir";
    } else if (line.toLowerCase().endsWith("vaetti")) line = line + "r";
    else if (line.toLowerCase().endsWith("avvite")) line = line.substring(
      0,
      line.length() - 3
    ) +
    "im";
    else if (
      line.toLowerCase().endsWith("siddhe") ||
      line.toLowerCase().endsWith("sidhe")
    ) {
      // do nothing
    } else line = line + "s";
  ...
  public static String capitalize(String s) {
    ...
    return Arrays.stream(s.split("\\s+"))
      .map(str ->
        DO_NOT_CAPITALIZE.contains(str)
          ? str
          : str.length() <= 1
            ? str.toUpperCase()
  ...
  public static List<String> parseArgs(
    String str,
    String separator,
    boolean leaveseparator
  ) {
    List<String> newlist = new ArrayList<>();

    int offset = 0;
    if (leaveseparator) offset = 1;
