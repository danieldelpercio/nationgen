...
public class NationGenDB {
  ...
  public void setDefinition(List<String> columnNames) {
    if (columnNames.size() != this.definition.size()) {
      throw new IllegalArgumentException(
        String.format(
          "Definition must be the same size (old = %s, new = %s)",
    ...
    if (!"id".equals(columnNames.get(0))) {
      throw new IllegalArgumentException(
        String.format(
          "First column name must be 'id' (found '%s' instead)",
          columnNames.get(0)
        )
      );
    }
  ...
  public List<String> getColumn(String name) {
    ...
    if (placeOfValue == -1) return list;

    for (List<String> line : entryMap.values()) list.add(
      line.get(placeOfValue)
    );
  ...
  public void saveToFile(String filename) {
    ...
    for (int i = 0; i < 5000; i++) {
      if (entryMap.keySet().contains("" + i)) lines.add(
        String.join(";", entryMap.get("" + i))
      );
    }
